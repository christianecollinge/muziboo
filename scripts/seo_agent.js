...
const TARGETS = [
	{
		...
		language: "English",
		instructions:
			"Write the post in English, focusing on the US music industry.",
	},
	{
		location: "United Kingdom",
		language: "English",
		instructions:
			"Write the post in British English, focusing on the UK music scene.",
	},
	{
		location: "Spain",
		language: "Spanish",
		instructions:
			"Write the post entirely in Spanish, focusing on the Spanish music scene.",
	},
	{
		location: "Germany",
		language: "German",
		instructions:
			"Write the post entirely in German, focusing on the German music scene.",
	},
];
...
async function fetchKeywords(target) {
	console.log(
		`🔍 Fetching keywords from DataForSEO for ${target.location} (${target.language})...`
	);

	if (!process.env.DATAFORSEO_LOGIN || !process.env.DATAFORSEO_PASSWORD) {
		console.warn(
			`⚠️ DataForSEO credentials are missing. Skipping API and falling back to static keywords.`
		);
		return (
			FALLBACK_KEYWORDS[target.language] || FALLBACK_KEYWORDS["English"]
		).slice(0, 1);
	}
	...
	try {
		...
		console.warn(
			`⚠️ DataForSEO returned no results. Using 1 fallback keyword.`
		);
		return (
			FALLBACK_KEYWORDS[target.language] || FALLBACK_KEYWORDS["English"]
		).slice(0, 1);
...
async function generateBlogPost(keyword, target, pubDate, retryCount = 0) {
	console.log(
		`✍️ Generating blog post for keyword: "${keyword}"... (Attempt ${retryCount + 1})`
	);
	...
	try {
		...
		if (
			...
			console.warn(
				`⚠️ Rate limit hit for "${keyword}". Waiting 5 minutes before retry...`
			);
...
async function main() {
	...
	if (missingVars.length > 0) {
		console.error(
			`❌ Missing environment variables: ${missingVars.join(", ")}. Please check your .env file or repository secrets.`
		);
		process.exit(1);
	}
		...
		if (markdown) {
			...
			if (image) {
				...
				if (localUrl) {
					...
					console.log(
						`🖼️ Downloaded relevant image: ${image.name} to ${localUrl}`
					);
				}
			}
			...
			console.log(
				`   Readability: ${readability.score} (Ease), Grade Level: ${gradeLevel}`
			);
		}
